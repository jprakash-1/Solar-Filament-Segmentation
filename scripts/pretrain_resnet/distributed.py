...
Verbatim port of `jp-mvp1:src/distributed.py` (not re-derived) -- that code
already went through real debugging against Kaggle 2xT4 hardware, see
`PRETRAIN_PLAN.md` section 4.3 and `RESNET_PRETRAIN_PLAN.md` section 0.

Deliberately not used unless launched with torchrun -- a plain `python
train_byol.py` still runs single-process (CPU/MPS/one GPU) exactly as
before (detected via the RANK env var torchrun sets; absent otherwise).
"""
